...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        centre = rg.Point(circle.center.x + (k * circle.radius), circle.center.y + (k * circle.radius))
        square = rg.Square(centre, 2 * circle.radius)
...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + '8 blue row, 3 in column; 4 green in row, 5 in column'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    widthcentre = ((rectangle.corner_1.x + rectangle.corner_2.x)/2)
    heightcentre = ((rectangle.corner_1.y + rectangle.corner_2.y)/2)
    if halfheight < 0:
        halfheight = -halfheight
    if halfwidth < 0:
        halfwidth = -halfwidth
    centre = rg.Point(widthcentre - halfwidth - halfheight, heightcentre)
    for k in range(m):
        circle = rg.Circle(centre, halfheight)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
        centre = rg.Point(centre.x - 2 * halfheight, heightcentre)

    centre = rg.Point(widthcentre, heightcentre - halfwidth - halfheight)
    for k in range(n):
        circle2 = rg.Circle(centre, halfwidth)
        circle2.outline_color = rectangle.outline_color
        circle2.attach_to(window)
        centre = rg.Point(widthcentre, centre.y - 2 * halfwidth, )
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    ####################################################################
    # ------------------------------------------------------------------
    halfwidth = ((rectangle1.corner_1.x - rectangle1.corner_2.x) / 2)
    halfheight = ((rectangle1.corner_1.y - rectangle1.corner_2.y) / 2)
    widthcentre1 = ((rectangle1.corner_1.x + rectangle1.corner_2.x) / 2)
    heightcentre1 = ((rectangle1.corner_1.y + rectangle1.corner_2.y) / 2)
    widthcentre2 = ((rectangle2.corner_1.x + rectangle2.corner_2.x) / 2)
    heightcentre2 = ((rectangle2.corner_1.y + rectangle2.corner_2.y) / 2)
    if halfheight > 0:
        halfheight = -halfheight
    if halfwidth < 0:
        halfwidth = -halfwidth
    start = rg.Point(widthcentre1, heightcentre1)
    end = rg.Point(widthcentre2, heightcentre2)
    ...
    for k in range(n):
        ...
        start = rg.Point(start.x - halfwidth, start.y - halfheight)
        end = rg.Point(end.x - halfwidth, end.y - halfheight)
    window.render()
